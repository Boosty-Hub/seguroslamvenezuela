...
### Captura de datos del contacto (migración 0057)
- Tool `guardar_datos_contacto` (system, gate `crm_can_update_contact`): el agente **completa proactivamente** el nombre completo / email / teléfono del **contacto** en Kommo apenas el lead los comparte (descripción de la tool dice "ÚSALA SIEMPRE…", a diferencia de las CRM tools de 0028 que son "solo cuando te lo indiquen"). Handler en `generate-response/runCrmTool`: **lee el contacto antes de escribir y SOLO completa lo que está VACÍO — nunca pisa un dato existente** (decisión del operador). Email/Teléfono son multitext estándar (`field_code` EMAIL/PHONE) → se escribe con `enum_code` resuelto de `fetchContactEnumCodes` (fallback `WORK`); el nombre es el `name` top-level del contacto. Helpers nuevos en `_shared/kommo.ts`: `fetchContactSnapshot` / `fetchContactEnumCodes` / `patchContactRaw`. Requiere `crm_actions_enabled=true` + `crm_can_update_contact=true`.
